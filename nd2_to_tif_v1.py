...
if type(to_extract) == str:
	...
	elif to_extract.endswith('.nd2'):
		filelist = [to_extract]
	else:
		print("Please make sure the input file is a '.nd2' file")
		exit()
	...
	for file in to_extract:
		if type(file) != str:
			...
			exit()
		elif not file.endswith('.nd2'):
			print("Please make sure all files in the input list are '.nd2' files")
			exit()
	filelist = to_extract
else:
...
dir_name = 'extracted_'+extension[1:]+'s'
if os.path.exists(os.path.join(inpath, dir_name))==False:
	os.mkdir(os.path.join(inpath, dir_name))
		
filelist = [x for x in filelist if x.endswith('.nd2')]
for filename in filelist:
	extract_from_nd2(filename, inpath, dir_name, convert_16_to_8bit, scale_image, auto_scale, scale_min, scale_max, z_stack, find_best_z, max_projection, auto_slice, start_slice, end_slice, return_composite, channel_order, return_channels, extension)
